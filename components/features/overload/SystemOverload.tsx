...
export function SystemOverload({ enabled, contentRoot }: Props) {
  ...
  const trigger = useCallback(() => {
    ...
    const root = contentRoot?.current ?? document.querySelector(".site-page__content");
    const textTargets = root
      ? Array.from(
          root.querySelectorAll<HTMLElement>(
            "h1,h2,h3,h4,p,.font-display,.font-label,li,.site-nav-link,footer p,.ink-border-thin span",
          ),
        ).filter((el) => el.offsetParent !== null && (el.textContent?.trim().length ?? 0) > 0)
      : [];

    const fallTargets = root
      ? Array.from(root.querySelectorAll<HTMLElement>("h1,h2,h3,.site-title-stroke,.font-display"))
      : [];
    ...
    <>
      ...
      {(phase === "chaos" || phase === "blackout") && (
        <div className="overload-stack" aria-live="assertive">
          ...
          {phase === "blackout" && (
            <button type="button" className="overload-blackout" onClick={reboot}>
