...
import { WANTED_SLAM_3D_INITIAL, WANTED_SLAM_SPRING } from "@/lib/motion/motion";
import { AmbientFloat3D } from "@/components/manga/effects/AmbientFloat3D";
import { Tilt3D } from "@/components/manga/effects/Tilt3D";
...
function CardInner({ className, slam }: Props) {
  ...
  return (
    <motion.div
      className={cn("hero-profile-card ink-border relative mx-auto w-full overflow-hidden p-5 md:p-6", className)}
      initial={slam && !reduced ? WANTED_SLAM_3D_INITIAL : false}
      animate={
        slam && !reduced
          ? { opacity: 1, scale: 1, y: 0, rotateX: 0, rotateY: 0, z: 0 }
          : undefined
      }
      transition={slam && !reduced ? WANTED_SLAM_SPRING : { duration: 0.01 }}
    >
      ...
      <p className="mt-3 text-center text-xs manga-text-dim">{profile.note}</p>
    </motion.div>
  );
}
