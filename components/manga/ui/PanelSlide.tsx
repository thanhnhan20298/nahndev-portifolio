...
import { useMangaReveal } from "@/hooks/useMangaReveal";
import {
  PANEL_DURATION,
  PANEL_LAND_SPRING,
  PANEL_OFFSET,
...
import { useMangaGsap } from "@/context/MangaGsapContext";
...
export function PanelSlide({
  ...
  const { enabled: gsapOn } = useMangaGsap();
  const gsapReveal = gsapOn && afterChapter && !depth3d;

  const { ref, show, reduced: revealReduced, panelDelaySec } = useMangaReveal({
    variant: "panel",
    afterChapter,
  });
  ...
  const panel = (
    <motion.div
      ...
      className={cn("min-w-0", depth3d && "manga-panel-3d-inner")}
  ...
  return (
    <div
      ref={ref}
      className={cn("manga-panel-scene", className)}
