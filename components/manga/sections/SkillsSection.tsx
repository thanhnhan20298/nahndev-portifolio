...
import { RadarArchitecture } from "@/components/manga/features/radar/RadarArchitecture";
...
export function SkillsSection({ scrollEnabled }: Props) {
  ...
  return (
    <section
      id="skills"
      className="radar-skills-section relative border-b-4 manga-border-section"
    >
      <div ref={pinRef} className="radar-skills-section__pin">
        <div className="radar-skills-section__head mx-auto max-w-6xl px-4 pt-8 pb-2 md:px-8">
          ...
          <p className="mt-2 max-w-xl text-sm manga-text-dim">{radarSection.subtitle}</p>
        </div>
          ...
          {archNodes
            ...
            .map((n) => (
              ...
              >
                ...
                <span className="font-label text-[9px] uppercase text-[var(--accent)]">{n.tier}</span>
              </li>
            ))}
        </ul>
