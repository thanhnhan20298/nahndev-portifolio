...
import { SectionShell } from "@/components/manga/layout/SectionShell";
import { SNAP_EASE, SLASH } from "@/lib/motion/motion";
import { ActionSwoosh } from "@/components/manga/effects/ActionSwoosh";
...
const tabs = [
  ...
  { id: "impact" as const, label: "Impact flash", hint: "Flash effect when opening a project page." },
  {
    ...
    >
      <div data-showcase-scene className="relative overflow-hidden">
        <div data-showcase-impact className="showcase-impact-burst" aria-hidden>
          <MangaSpeedlines className="opacity-50" strong />
        </div>
        <PanelSlide from="left" afterChapter>
          ...
          <p className="mt-3 text-[11px] leading-relaxed manga-text-dim">{active.hint}</p>
          ...
          <div className="mt-6 grid gap-6 md:grid-cols-[auto_1fr] md:items-center">
            <button
              ...
              className="manga-cta px-5 py-2.5 text-sm font-bold"
            >
              {actionLabel(tab)}
            </button>
            ...
            >
              ...
              {tab === "impact" && (
                <p className="relative z-[1] text-xs leading-relaxed">
                  Impact ~{SLASH.impact.totalMs}ms — slash, shrink, flash (synced with project open).
                </p>
              )}
              {tab === "pipeline" && (
                <ul className="space-y-2 font-mono text-xs leading-relaxed">
                  {pipeline.map((label, i) => (
                    <li
                      key={label}
                      className={cn(step >= i ? "font-bold text-[var(--accent)]" : "opacity-40")}
                    >
                      {step >= i ? "▸" : "○"} {label}
                    </li>
                  ))}
