...
import { PanelSlide, PANEL_SLIDE_DELAY } from "@/components/manga/ui/PanelSlide";
import { PanelStagger } from "@/components/manga/ui/PanelStagger";
import { InkPanel } from "@/components/manga/ui/InkPanel";
import { AwardBadges } from "@/components/manga/ui/AwardBadges";
import { SectionShell } from "@/components/manga/layout/SectionShell";
import { experience } from "@/lib/content/experience";

function ExperienceCard({ arc, showAwards }: { arc: (typeof experience)[0]; showAwards?: boolean }) {
  return (
    <InkPanel clip={showAwards ? "diag-tr" : "diag-bl"} className="h-full p-6 md:p-8">
      <PanelStagger>
        <span className="font-label text-xs text-[var(--accent)]">
          {arc.arc}
        </span>
        ...
        <p className="mt-1 text-sm font-bold manga-text-dim">{arc.role}</p>
