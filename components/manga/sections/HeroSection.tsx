...
import { SpeechBubble } from "@/components/manga/ui/SpeechBubble";
import { InkPanel } from "@/components/manga/ui/InkPanel";
import { MangaTerminal } from "@/components/manga/ui/MangaTerminal";
import { MangaSpeedlines } from "@/components/manga/ui/MangaSpeedlines";
import { StaggeredTitle } from "@/components/manga/ui/StaggeredTitle";
import { PanelSlide } from "@/components/manga/ui/PanelSlide";
import { HeroImpactBurst } from "@/components/manga/features/hero/HeroImpactBurst";
import { HeroAgentHQ } from "@/components/manga/features/hero/HeroAgentHQ";
import { HeroProfileCard } from "@/components/manga/features/hero/HeroProfileCard";
...
export function HeroSection({ booted }: Props) {
  ...
  return (
    <section
      data-hero-scene
      className="manga-hero-scene hero-agent-hq relative overflow-hidden border-b-4 manga-border-section"
    >
      ...
      <div data-hero-layer="speedlines" className="hero-layer hero-layer--speedlines pointer-events-none">
        <MangaSpeedlines className="manga-speedlines--live opacity-35" strong={burst} />
      </div>
      ...
      <div data-hero-layer="agent-3d" className="hero-layer hero-layer--agent relative z-[2]">
        ...
        <div className="hero-agent-hq__badge pointer-events-none">
          ...
          <p className="font-display manga-title-stroke mt-1 text-3xl uppercase md:text-4xl">
            {heroHQ.role}
          </p>
        </div>
      </div>
      ...
      <div data-hero-layer="content" className="relative z-[4] mx-auto max-w-6xl px-4 pb-14 pt-4 md:px-8 md:pb-20">
        <PanelSlide from="bottom" delay={0}>
          <div className="mb-4 flex flex-wrap items-center gap-2">
            <span className="manga-cta px-3 py-1 text-xs font-black uppercase">
              {about.availability}
            </span>
            <span className="ink-border-thin bg-panel px-3 py-1 text-xs font-bold">{about.location}</span>
            <span className="manga-arc-badge">{heroChapter.arc}</span>
          </div>
        </PanelSlide>
        ...
        <div className="grid gap-6 lg:grid-cols-2 lg:items-start">
          <PanelSlide from="left" delay={0.08}>
            <InkPanel large clip="diag-tr" className="relative p-6 md:p-8">
              ...
              <h1 className="font-display manga-title-stroke mt-3 text-5xl uppercase md:text-6xl lg:text-7xl">
